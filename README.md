...
> 5分で概要を知りたい方は§1〜3を、技術的な詳細は各`strategies/*/README.md`と[REQUIREMENTS.md](REQUIREMENTS.md)を参照してください。
...
- [2. 設計思想の核](#2-設計思想の核)
- [3. 主要な発見](#3-主要な発見)
  - [3-1. 多重検定問題（ペアトレード）](#3-1-多重検定問題ペアトレード)
  - [3-2. ポジションサイジングのスケール見落とし（マルチファクター）](#3-2-ポジションサイジングのスケール見落としマルチファクター)
  - [3-3. セクター中立化：設計目的の達成とバックテスト成績のトレードオフ](#3-3-セクター中立化設計目的の達成とバックテスト成績のトレードオフ)
  - [3-4. VRPシグナルは単純に持ち続けるより明確に悪い結果を生んだ（Vol Arbitrage）](#3-4-vrpシグナルは単純に持ち続けるより明確に悪い結果を生んだvol-arbitrage)
  - [3-5. 可視化：Walk-Forwardとパラメータ感度分析](#3-5-可視化walk-forwardとパラメータ感度分析)
  - [3-6. 生成済みレポート](#3-6-生成済みレポート)
  - [3-7. コードレビューで発覚した、検証機構自体の欠陥（assert_backtest_causalの1バー検出漏れ）](#3-7-コードレビューで発覚した検証機構自体の欠陥assert_backtest_causalの1バー検出漏れ)
  - [3-8. コードレビューで発覚した、有意性判定ロジックの裾ミスマッチ（core/evaluation/）](#3-8-コードレビューで発覚した有意性判定ロジックの裾ミスマッチcoreevaluation)
  - [3-9. コードレビューで発覚した、多重検定補正そのものの過小評価（strategies/）](#3-9-コードレビューで発覚した多重検定補正そのものの過小評価strategies)
- [4. 技術スタック・アーキテクチャ概要](#4-技術スタックアーキテクチャ概要)
- [5. 再現手順](#5-再現手順)
...
検証結果はStreamlitのインタラクティブダッシュボードで確認できる（§5-1）。外部コードレビュー（`/code-review`）により検証基盤自体に複数の欠陥が見つかり、修正した経緯も記録している（§3-7〜3-9）。
